...
if __name__ == "__main__":
    ...
    plt.clf()
    reds2 = np.array([8, 6, 5, 4, 3])
    plot_smhms(reds2, outdir=simdir, metal=True)
    plt.clf()
    plot_smhms(reds2, outdir=simdir, star=False, metal=True)
    plt.clf()
    plot_smhms(reds2, outdir=simdir, star=False, hdf5=False,allbar=True)
    plt.clf()
    plot_smhms(reds2, outdir=simdir, star=False, hdf5=False)
    plt.clf()
    reds2 = np.array([10, 8, 6,4,3])
    dataf = ["data/stellar-to-halo-mass-thesan-z10.txt", "data/stellar-to-halo-mass-thesan-z8.txt", "data/stellar-to-halo-mass-thesan-z6.txt"]
    plot_thesan_smhm(dataf[2], color="magenta", ls="-.", red=6)
    plot_thesan_smhm(dataf[1], color="green", ls=":", red=8)
    plot_thesan_smhm(dataf[0], color="green", ls="-.", red=10)
    plot_smhms(reds2, outdir=simdir)
    plt.clf()
    reds2 = np.array([4, 3])
    plot_smhms(reds2, outdir=simdir)
    plt.clf()
    plot_smhms_reion(reds2, outdir=simdir, metal=True)
    plt.clf()
    plot_smhms_reion(reds2, outdir=simdir, star=False, metal=True)
    plt.clf()
    plot_smhms_reion(reds2, outdir=simdir)
    plt.clf()
    plot_smhms_reion(reds2, outdir=simdir, star=False)
    plt.clf()
    plot_smhms_conc(np.array([4,]), outdir=simdir)
    plt.clf()
